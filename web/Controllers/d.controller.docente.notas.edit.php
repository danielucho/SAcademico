...
$sentencia = $pdo -> prepare("SELECT * FROM nota WHERE inscripcion_id = :inscripcion_id");
$sentencia -> bindParam(':inscripcion_id', $inscripcion_id);
$sentencia -> execute();
$ConsultaNota = $sentencia -> fetch(PDO::FETCH_ASSOC);
...
switch ($accion) {
    ...
    case 'btnAceptar':
        $sentencia = $pdo -> prepare("UPDATE nota SET nota = :nota, observacion = :observacion WHERE id = :id");
